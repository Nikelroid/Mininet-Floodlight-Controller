...
//        }
    ...
    private List<Path> yens(DatapathId src, DatapathId dst, Integer K, Archipelago aSrc, Archipelago aDst) {
        ...
        // Find link costs
        Map<Link, Integer> linkCost = initLinkCostMap(MalinkCost);
        if (!WP) {
            System.out.println("COSTS:" + linkCost);
            WP = true;
        }
        ...
        int cst = 0;
        if (A.get(0).getId().getSrc().toString().equals("00:00:00:00:00:00:00:01") &&
                A.get(0).getId().getDst().toString().equals("00:00:00:00:00:00:00:02")) {
            int s = A.get(0).getPath().size();
            System.out.println("\n______________\n");
            System.out.println(A.get(0).getId().getSrc().toString().substring(21)
                    + " ---> " + A.get(0).getId().getDst().toString().substring(21));

            System.out.print("\nSTART- \n"+A.get(0).getPath().get(0).getNodeId().toString().substring(21)
                    +", port:"+A.get(0).getPath().get(0).getPortId() +"\n");
            Link ll = new Link();

            for (int i = 1; i < s-2; i=i+2) {
                System.out.print(A.get(0).getPath().get(i).getNodeId().toString().substring(21)+
                        ", from port: "+A.get(0).getPath().get(i).getPortId()
                        +" to port: "+A.get(0).getPath().get(i+1).getPortId() );
                ll.setSrc(A.get(0).getPath().get(i-1).getNodeId());
                ll.setDst(A.get(0).getPath().get(i).getNodeId());
                ll.setSrcPort(A.get(0).getPath().get(i-1).getPortId());
                ll.setDstPort(A.get(0).getPath().get(i).getPortId());
                System.out.print(" --cost-between ("+A.get(0).getPath().get(i-1).getNodeId().toString().substring(21)+
                        ") and ("+
                        A.get(0).getPath().get(i).getNodeId().toString().substring(21)
                        +") = "+MalinkCost.get(ll));
                cst+=MalinkCost.get(ll);
                System.out.print(" --cum-cost="+cst+"\n");

            }
            int ss = A.get(0).getPath().size();
            System.out.print(A.get(0).getPath().get(s-1).getNodeId().toString().substring(21)+
                    ", port:"+A.get(0).getPath().get(s-1).getPortId() );
            ll.setSrc(A.get(0).getPath().get(ss-2).getNodeId());
            ll.setDst(A.get(0).getPath().get(ss-1).getNodeId());
            ll.setSrcPort(A.get(0).getPath().get(ss-2).getPortId());
            ll.setDstPort(A.get(0).getPath().get(ss-1).getPortId());
            System.out.print(" --cost-between ("+A.get(0).getPath().get(s-2).getNodeId().toString().substring(21)+
                    ") and ("+
                    A.get(0).getPath().get(s-1).getNodeId().toString().substring(21)
                    +") = "+MalinkCost.get(ll));
            cst+=MalinkCost.get(ll);
            System.out.print(" --cum-cost="+cst+"\n");
            System.out.print( " - END\n");
            System.out.println("Final Cost: " + cst+"\n\n");

        }
        List<Path> BestRoute = new ArrayList<Path>();
        BestRoute.add(A.get(0));
